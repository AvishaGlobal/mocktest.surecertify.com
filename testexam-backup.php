...
<?php
if($_REQUEST['QLang']){ $_SESSION['PLang']=$_REQUEST['QLang']; }
...
<link href="teststyle.css" rel="stylesheet" type="text/css" />
 ...
 document.onmouseup = mousehandler;
  ...
  .clears{padding:8px 20px; background:#3B5998; margin-top:4px; border-radius:4px; color:#FFF!important; display:inline-block!important; float:left; margin-right:20px; margin-bottom:8px;}
  .top-a{ display:block; float:left; padding:4px 20px; background:#3B5998; text-transform:uppercase; font-size:12px; font-weight:bold; color:#fff; margin:5px 10px; border-radius:4px;}
  .qnpaper {
    width: 100%;
    /*overflow: auto;*/
    float: left;
...
.left-box { position:fixed; height:1000px;}
.
...
<form id="frmfinaltest" name="frmfinaltest" method="post" action="FinalTestResults.php?TsIDs=<?php echo $_REQUEST['TsIDs']?>&mdlev=<?php echo $_REQUEST['mdlev']?>">
  <tr>
    <td width="20%" valign="top" bgcolor="#6E91B9"><table class="left-box"  border="0" cellspacing="1" cellpadding="1" style="width:270px; ">
      <tr>
        <td  height="60" align="center" valign="middle" bgcolor="#0033FF"><h4 style="color:#fff;margin: 0;padding: 14px 15px;font-size: 22px;"><font>My</font> <span>Certification</span></h4></td>
      </tr>
      <tr>
        <td align="left" valign="middle" bgcolor="#6e91b9" style="border-right:1px solid #000000; color:#FFFFFF; font-weight:bold; height:25px;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td width="37%" align="center">Time Left </td>
            <td width="63%"><div id="javascript_countdown_time"></div></td>
          </tr>
        </table></td>
      </tr>
      
      <tr>
        <td align="left" valign="top" bgcolor="#6e91b9" style="border-right:1px solid #000000; color:#FFFFFF; font-weight:bold;"><table>
            <tr height="20" width="100%" class='pages'>
              <td><div id="sideNav">
                  <table class="legentlist">
                    <tr class="trborder">
                      <td valign="middle"><span class="">&nbsp;</span></td>
                      <td valign="middle"><b>Legends</b></td>
                    </tr>
                    <tr class="trborder">
                      <td valign="middle"><span class="solved">&nbsp;</span></td>
                      <td>Attempted</td>
                    </tr>
                    <tr class="trborder">
                      <td valign="middle"><span class="marked">&nbsp;</span></td>
                      <td>Review Later</td>
                    </tr>
                    <tr>
                      <td valign="middle"><span class="unsolved">&nbsp;</span></td>
                      <td>Un-attempted</td>
                    </tr>
                  </table>
              </div></td>
	...
	while($SnumquesArr = $SnumquesRes->fetch_assoc()){
		...
		?>
		<div><a id='qn-<?php echo $vsv?>' class='<?php echo $Clss?>' onclick="location.href='testexam.php?TsIDs=<?php echo $_REQUEST['TsIDs']?>&Pageno=<?php echo $vsv?>&QLang=<?php echo $_REQUEST['QLang']?>'"><?php echo $vsv?></a></div>
	<?php } ?>  
            ...
            <tr height="20" width="100%" class='pages'>
              <td id="Suggestions"><input name="TsIDs" type="hidden" id="TsIDs" value="<?php echo $_REQUEST['TsIDs']?>" />
                <input name="mdlev" type="hidden" id="mdlev" value="<?php echo $_REQUEST['mdlev']?>" /></td>
            </tr>
            
            <tr height="20" width="100%" class='pages'>
              <td valign="bottom"><iframe src="updatetimer.php?stime=<?php echo $Timing?>&TestIDds=<?php echo $TestIDds?>&Testlev=<?php echo base64_decode($_REQUEST['mdlev'])?>" width="1" height="1" allowtransparency="yes" style="border:0px;"></iframe></td>
            </tr>
        </table></td>
      </tr>
    </table></td>
    <td width="80%" valign="top"><table width="100%" border="0" cellspacing="1" cellpadding="1">
      ...
      <tr>
	   ...
	   ?>
        <a href="testexam.php?TsIDs=<?php echo $_REQUEST['TsIDs']?>&Pageno=<?php echo $Selsubsarr['qfrom']?>" class="top-a" style="font-size:10px;"><?php echo $Selsubsarr['subject_name']?></a>
        <?php } ?>
        ...
        <strong>Language :</strong> &nbsp;<select name="paperLang" id="paperLang" class="frm-title-box" style="height:25px;" onchange="javascript:location.href='<?php echo $CurUrl?>'">
            <option value="E" <?php if($_SESSION['PLang']=='E'){ ?> selected="selected" <? } ?>>English</option>
            <option value="H" <?php if($_SESSION['PLang']=='H'){ ?> selected="selected" <? } ?>>Hindi</option>
        </select><?php } ?></div></td>
      </tr>
	...
	$Seloptq = "select * from quizmng_tbl where q_session = '".$_SESSION['q_ses']."' and memid = '".$_SESSION['StudId']."' and ques_id = '".$SqustionArr['tqid']."'";
		...
		#######################################################################
 	?>
        <a id='firstqn'></a><div class='fullQ' id='q_1'><div class='qHead'><table style="margin-bottom:60px;">
        <tr><td colspan="2"><div class='answer' style="font-size:12p; font-weight:bold; height:25px; padding:5px; width:99%;">  Question No. <?php echo $sns++?>   </div></td></tr>
		<tr><td><div id='q_txt_1' style=""><div class='question'> 
          <p>
            ...
            <?php }else{ ?>
            <p> <?php echo $SqustionArr['question']?></p>
             
          
		  <table width="100%" cellspacing="2" cellpadding="2" border="0">
            <tr style="height:30px;">
              <td width="5%"><strong>A</strong></td>
              <td width="65%"><?php echo $SqustionArr['option1']?></td>
              <td width="5%"><strong>B</strong></td>
              <td width="25%"><?php echo $SqustionArr['option2']?></td>
            </tr>
            
            <tr style="height:30px;">
              <td><strong>C</strong></td>
              <td><?php echo $SqustionArr['option3']?></td>
              <td><strong>D</strong></td>
              <td><?php echo $SqustionArr['option4']?></td>
            </tr>
          </table>
		...
		</div><div class='tdanswer'><div class='optionss' > <strong>Answer:</strong> <br />
    ...
    <input type='radio' onclick='updateAnswer(<?php echo $sns-1?>, <?php echo $i?>, this.value, <?php echo $SqustionArr['tqid']?>)' id='ans-<?php echo $sns-1?>-<?php echo $i?>' name='ans-<?php echo $sns-1?>' align='left' value='<?php echo $i?>' <? if($Correctans==$i){?> checked="checked" <? } ?>><span class="ab"> <?php echo chr($Chars+$i)?></span>
    <br />
		<?php } ?>
        </div>
		 </div></div></div></td>
			<td class='q_rt'><div class='marks'><?php echo number_format($QuesMArks,1)?> Mark/s</div><div class='paraview'></div></td>
		</tr>
		<tr>
		  <td class='q_txt' colspan="2">&nbsp;</td>
 		  </tr>
        </table>
        </div><div class='answer'> <div id='answer-area' style="width:99%;">
		
		<div class=''><div class='answerclear'>
       ...
       <a href="javascript:void(0)" class='clears' onclick="visitLater(<?php echo $sns-1?>, <?php echo $SqustionArr['tqid']?>, 1)">Mark For Review </a>
		...
		<div class='tdvisitL'><div id='vl_<?php echo $sns-1; ?>' class='visitLaters'><input onclick='visitLater(<?php echo $sns-1; ?>, <?php echo $SqustionArr['tqid']; ?>, this.id)' type='hidden' name='later' value='Mark' id ='visitLater_<?php echo $sns-1; ?>' > </div></div>
		</div></div></div>
		<?php } ?>
    </div></div></td>
      </tr>
